    ...
    public partial class FrmTCPServer : Form
    {
        ...
        private void StartServerToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    //tcpServer.InitSocket(IPAddress.Parse("120.27.134.154"), port);
                    tcpServer.InitSocket(IPAddress.Parse("192.168.31.209"), port);
        ...
        private void insertsql(string msg)
        {

            JObject obj = JObject.Parse(msg);
            string jsonType = obj["jsonType"].ToString();

            if (jsonType=="6")
            { 
                string productMac = obj["productMac"].ToString();
                string shortAddr = obj["shortAddr"].ToString();
                ...
                string temp = obj["temp"].ToString();
                string TimeNow = DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss");
                string mac = productMac.Replace(":", ":");


                //string constr = "server=localhost;User Id=root;password=root;Database=socket";
                string constr = "server=120.27.134.154;User Id=test;password=testuser;Database=test";
                MySqlConnection mycon = new MySqlConnection(constr);
                mycon.Open();
                MySqlCommand mycmd = new MySqlCommand("insert into control_data(ID,jsonType,productMac,shortAddr,data,time,location,longitude,latitude,speed,direction,temp,TimeNow) values (uuid(),'" + jsonType + "','" + mac + "','" + shortAddr + "','" + data + "','" + time + "','" + location + "','" + longitude + "','" + latitude + "','" + speed + "','" + direction + "','" + temp + "','" + TimeNow + "')", mycon);
